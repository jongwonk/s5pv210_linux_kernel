				...
				int min_uV, int max_uV,unsigned *selector)
{
	...
	return max8698_set_voltage(rdev, min_uV, max_uV,selector);
}
...
static int max8698_set_suspend_voltage(struct regulator_dev *rdev, int uV)
{
	...
	case MAX8698_BUCK1 ... MAX8698_BUCK3:
		return max8698_buck_set_voltage(rdev, uV, uV,&selector);
	case MAX8698_LDO1 ... MAX8698_LDO9:
		return max8698_set_voltage(rdev, uV, uV,&selector);
	default:
		return -EINVAL;
	}
}
