...
#define READ_INT ((readl(S5PV210_GPH0DAT)>>5)&0x1)
struct {
...
static void kxtf9_irq_work_func(struct work_struct *work)
{
	...
	struct kxtf9_data *tf9 = container_of(work,struct kxtf9_data, irq_work);
	if (READ_INT) {
		status = INT_SRC_REG2;
...
static int kxtf9_misc_read(struct file *file, char __user *buffer,
...
/*	
	xyz[ABS_X] = tf9->input_dev->abs[ABS_X];
	xyz[ABS_Y] = tf9->input_dev->abs[ABS_Y];
	xyz[ABS_Z] = tf9->input_dev->abs[ABS_Z];
*/	
	xyz[ABS_X] = tf9->input_dev->absinfo[ABS_X].value;
	xyz[ABS_Y] = tf9->input_dev->absinfo[ABS_Y].value;
	xyz[ABS_Z] = tf9->input_dev->absinfo[ABS_Z].value;
	
	spin_unlock_irqrestore(&tf9->input_dev->event_lock, flags);

	if (copy_to_user(buffer, &xyz, n))
		return -EFAULT;

	return n;
}

static int kxtf9_misc_ioctl(struct file *file,unsigned int cmd, unsigned long arg)
{
	...
	case KXTF9_IOCTL_SET_FUZZ:
		...
		if (interval < 0)
			return -EINVAL;
		spin_lock_irqsave(&tf9->input_dev->event_lock, flags);
		tf9->input_dev->absinfo[ABS_X].fuzz = interval;
		tf9->input_dev->absinfo[ABS_Y].fuzz = interval;
		tf9->input_dev->absinfo[ABS_Z].fuzz = interval;
		spin_unlock_irqrestore(&tf9->input_dev->event_lock, flags);
		break;
	case KXTF9_IOCTL_SET_XYZ_HISTORY:
		if (copy_from_user(&interval, argp, sizeof(interval)))
			return -EFAULT;
		spin_lock_irqsave(&tf9->input_dev->event_lock, flags);
		tf9->input_dev->absinfo[ABS_X].value = interval;
		tf9->input_dev->absinfo[ABS_Y].value = interval;
		tf9->input_dev->absinfo[ABS_Z].value = interval;
		spin_unlock_irqrestore(&tf9->input_dev->event_lock, flags);
		break;
	default:
		return -EINVAL;
			   ...
			   const struct i2c_device_id *id)
{
	...
	err = request_irq(tf9->irq, kxtf9_isr, IRQ_TYPE_EDGE_RISING,"kxtf9_irq", tf9);
	if (err < 0) {
